//小弟不才 程式寫很髒請見諒 如有問題可以發問
//如有程式碼邏輯問題87%是小弟考慮不周畢竟是邊寫邊想的很大機率有遺漏 
//目前重新登使用者後bug連連...
...
int chat_friend() {
	...
	while(yptr!=NULL){
		if(strcmp(fname,yptr->fname)!=0 && yptr->fnext!=NULL){// bug 
			yptr=yptr->fnext;		
		}
...
int delete_friend() {
	...
	while(yptr!=NULL){
		...
		if(strcmp(fname,yptr->fname)==0){//如果相符合黑掉 
			printf("此後他的訊息將不會顯示!\n");
			break;
		} 
	}
...
int off_line(){//實作連線因該會要有ex:線上第幾人的問題 下線 
	...
	if(strcmp(ohead->name,lptr->name)==0&&ohead->onoff==1){//上線者第一筆資料就是本地使用者//但不是是用者的可能性.... 
		ohead=ohead->next;
		lhead=lhead->lnext;
